...
def test_text_nodes_with_same_url_are_equal() -> None:
    node1 = TextNode("test", TextType.PLAIN, "http//:test.com")
    node2 = TextNode("test", TextType.PLAIN, "http//:test.com")
    assert node1 == node2
...
def test_text_nodes_with_different_text_are_not_equal() -> None:
    node1 = TextNode("test", TextType.PLAIN)
    node2 = TextNode("oops", TextType.PLAIN)
    assert node1 != node2


def test_text_nodes_with_different_urls_are_not_equal() -> None:
    node1 = TextNode("test", TextType.LINK, "http//:test.com")
    node2 = TextNode("test", TextType.LINK, "https//:test.com")
    assert node1 != node2
